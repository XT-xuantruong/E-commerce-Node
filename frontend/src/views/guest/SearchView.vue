...
const fetchProducts = async () => {
  ...
  try {
    ...
    products.value = response.data.data.filter((product) =>
      product.name.toLowerCase().includes(route.query.q.toLowerCase())
    );
        ...
        <div class="mb-6">
          ...
          <div class="flex space-x-2">
            <input
              ...
              v-model="minPrice"
              placeholder="Min"
              class="w-1/2 p-2 border rounded focus:ring-2 focus:ring-blue-500"
              min="0"
            />
            <input
              type="number"
