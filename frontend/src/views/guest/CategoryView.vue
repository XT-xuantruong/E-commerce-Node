<template>
  <SidebarLayout>
    ...
    >
      ...
      >
        <div
          class="absolute right-0 top-0 h-full w-80 bg-white p-6 shadow-lg transform transition-transform"
          :class="{
            'translate-x-0': openFilterModal,
            'translate-x-full': !openFilterModal,
          }"
        >
...
const fetchCategory = async () => {
  try {
    const response = await categoryServices.get(route.params.categorySlug);
...
const fetchProducts = async () => {
  if (!category.value.category_id) return;
  ...
  try {
    const response = await productServices.gets({
      ...
      filter: ["category", category.value.category_id],
      sort: sortOrder.value,
    });


    productsByCategory.value = response.data.data.filter(
      (product) => product.category_id === category.value.category_id
    );
    console.log("hay hay");
