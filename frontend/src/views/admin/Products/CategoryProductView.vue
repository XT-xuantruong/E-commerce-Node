...
const handleDelete = async (id) => {
  if (window.confirm("Are you sure you want to delete this category?")) {
    try {
      ...
      categories.value = categories.value.filter(category => category._id !== id); // Xóa khỏi danh sách cục bộ
    } catch (error) {
              ...
              <tr v-for="category in filteredCategories" :key="category._id"
                ...
                <td class="py-5 px-4">
                  <div class="flex items-center space-x-3.5">
                    <button @click="openEditModal(category)" class="text-yellow-600 hover:text-yellow-900">
                    ...
                    <button @click="handleDelete(category._id)" class="text-red-600 hover:text-red-900">
                      <svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
          ...
          <div class="mt-2">
            <form @submit.prevent="saveCategory" class="space-y-4">
              <div class="form-control">
                ...
                <input v-model="currentCategory.title" type="text" required
