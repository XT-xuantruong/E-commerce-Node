    ...
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        ...
        <div class="flex-shrink-0">
          <RouterLink to="/" class="text-2xl font-bold text-indigo-600">E-Shop</RouterLink>
        </div>

        <!-- Search Bar -->
        <div class="flex-1 max-w-lg mx-8">
          <div class="relative">
            <input @keyup.enter="handleSearch" type="text" v-model="searchQuery" placeholder="Tìm kiếm sản phẩm..."
            ...
            <button @click="handleSearch" class="absolute right-3 top-2.5 text-gray-400 hover:text-indigo-600">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div v-if="!userStore.user.isAuthenticated" class="flex items-center space-x-2">
          ...
          <div v-else class="relative">
            <button @click="toggleDropdown" class="flex items-center space-x-2 focus:outline-none">
              <img :src="'http://127.0.0.1:8088' + userStore.user.avatar" alt="User avatar"
                class="w-8 h-8 rounded-full object-cover border-2 border-indigo-600" />
              <svg :class="{ 'rotate-180': isDropdownOpen }" class="w-4 h-4 transition-transform duration-200"
