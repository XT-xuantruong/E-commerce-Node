...
onBeforeMount(async () => {
  try {
    ...
    await productServices.gets().then((reponse) => {
      countProduct.value = reponse.data.data.length;
    });
...
const cardItems = computed(() => [
  {
    icon: `<svg class="fill-primary dark:fill-white" width="20" height="22" viewBox="0 0 20 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    total: `${formatCurrency(totalPriceOrder.value).toLocaleString()}`,
  },
  {
    ...
    title: "Total number Product",
    total: countProduct.value.toLocaleString(),
  },
  {
    icon: `<svg class="fill-primary dark:fill-white" width="22" height="18" viewBox="0 0 22 18" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    total: countUser.value.toLocaleString(),
  },
]);
</script>
...
<template>
  ...
  <div
    ...
    :key="index"
    class="rounded-sm border border-stroke bg-white py-6 px-7.5 shadow-default dark:border-strokedark dark:bg-boxdark flex flex-col items-center justify-center text-center"
  >
    <div
